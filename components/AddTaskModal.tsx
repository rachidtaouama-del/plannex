...
import { CheckCircle2, XCircle, PenTool, Activity, CheckCircle, ShieldCheck } from 'lucide-react';
...
interface AddTaskModalProps {
    ...
    onClose: () => void;
    onSave: (
        newTasks: Omit<SchedulingTaskData, 'id' | 'isScheduled' | 'START DATE' | 'END DATE' | 'TYPE D\'EQUIPE' | 'EQUIPE NUMBER' | 'MAX HOUR' | 'isKeyEvent' | 'predecessor'>[],
        insertAfterId: number | null
    ) => void;
    allTasks: SchedulingTaskData[];
    costData?: any[];
    costHubEntries?: CostHubEntry[]; // New prop for Cost Hub lookups
}

const initialFormState: Partial<SchedulingTaskData> = {
    'GLOBAL TASKS': '',
    'Nom Equipement': '',
    'FAMILLE': '',
    'DISCIPLINE': '',
    ...
    'OT': 'NA',
    'COMMENTAIRE HSE': 0,
    'Type de Maintenance': 'PREVENTIVE',
    'COMPANY': '',
    'POSTE NUMBER': '',
    'POSTE DESCRIPTION': '',
    'QT': 0,
...
export const AddTaskModal: React.FC<AddTaskModalProps> = ({ isOpen, onClose, onSave, allTasks, costData, costHubEntries }) => {
    ...
    useEffect(() => {
        if (mode === 'single') {
            const duration = formData['DUREE'] || 0;
            const manpower = formData['EFFECTIF'] || 1;
            setFormData(prev => ({
                ...prev,
                'Heures-Homme': parseFloat((duration * manpower).toFixed(2))
            }));
        }
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
        ...
        const numericSelects = [
            'COMMENTAIRE HSE', 'MO Required', 'MO Readiness', 'ADRPT Required', 'ADRPT Readiness',
            'QT', 'Additional Cost'
        ];
    ...
    const addDiscipline = () => {
        setMultiDisciplineData(prev => [...prev, { id: crypto.randomUUID(), discipline: '', effectif: 1 }]);
    };

    const updateDiscipline = (id: string, field: 'discipline' | 'effectif', value: string | number) => {
        setMultiDisciplineData(prev => prev.map(item => item.id === id ? { ...item, [field]: value } : item));
    };

    const removeDiscipline = (id: string) => {
        setMultiDisciplineData(prev => prev.filter(item => item.id !== id));
    };

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();

        const newTasks: Omit<SchedulingTaskData, 'id' | 'isScheduled' | 'START DATE' | 'END DATE' | 'TYPE D\'EQUIPE' | 'EQUIPE NUMBER' | 'MAX HOUR' | 'isKeyEvent' | 'predecessor'>[] = [];

        const baseTaskData = {
            'GLOBAL TASKS': formData['GLOBAL TASKS'] || '',
            'Nom Equipement': formData['Nom Equipement'] || '',
            'FAMILLE': formData['FAMILLE'] || '',
            'Type de Maintenance': formData['Type de Maintenance'] || 'PREVENTIVE',
            'DUREE': formData['DUREE'] || 0,
            'AVIS': formData['AVIS'] || 'NA',
            'OT': formData['OT'] || 'NA',
            'COMMENTAIRE HSE': formData['COMMENTAIRE HSE'] || 0,
            'MO Required': formData['MO Required'] || 0,
            'MO Readiness': formData['MO Readiness'] || 0,
        ...
        if (mode === 'single') {
            ...
            const effectif = formData['EFFECTIF'] || 1;
            if (!discipline) {
                setAlertConfig({
                    isOpen: true,
                    title: 'Discipline Manquante',
                    message: 'Veuillez sélectionner une discipline pour cette tâche.'
                });
                return;
            }
            newTasks.push({
                ...
                EFFECTIF: effectif,
                'Heures-Homme': (baseTaskData.DUREE * effectif),
            });
        } else {
            if (multiDisciplineData.length === 0 || multiDisciplineData.some(d => !d.discipline)) {
                setAlertConfig({
                    isOpen: true,
                    title: 'Configuration Multi-Disciplines',
                    message: 'Veuillez ajouter au moins une discipline et renseigner tous les champs.'
                });
                return;
            }
            const multiDisciplineId = crypto.randomUUID();
            multiDisciplineData.forEach(item => {
                newTasks.push({
                    ...baseTaskData,
                    DISCIPLINE: item.discipline,
                    EFFECTIF: item.effectif,
                    'Heures-Homme': (baseTaskData.DUREE * item.effectif),
                    multiDisciplineId: multiDisciplineId,
                });
            });
        }

        onSave(newTasks as any[], insertAfterId);
        onClose();
    };
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex justify-center items-center z-[70] p-4 animate-in fade-in duration-300 overflow-y-auto">
            <div className="bg-slate-900 rounded-[2.5rem] shadow-2xl w-full max-w-4xl border border-white/10 my-auto animate-in zoom-in-95 duration-500 flex flex-col max-h-[95vh]">
                ...
                <header className="flex justify-between items-center p-8 pb-4 shrink-0">
                    <div>
                        <h2 className="text-2xl font-black text-white tracking-tight uppercase">Ajouter une Nouvelle Tâche</h2>
                        <p className="text-[10px] text-slate-500 font-bold uppercase tracking-[0.2em] mt-1 shrink-0">Configuration du Planning Opérationnel</p>
                    </div>
                ...
                <form id="add-task-form" onSubmit={handleSubmit} className="p-8 pt-4 space-y-8 overflow-y-auto flex-1 custom-scrollbar">
                    ...
                    {/* SECTION: SEQUENCING */}
                    <div className="bg-slate-950/40 p-6 rounded-[2rem] border border-blue-500/20">
                        <div className="flex items-center gap-3 mb-4">
                            <PenTool className="w-4 h-4 text-blue-400" />
                            <h3 className="text-[11px] font-black text-white uppercase tracking-wider">Positionnement Séquentiel</h3>
                        </div>
                        <TaskSelector
                            tasks={allTasks}
                            selectedIds={insertAfterId ? [insertAfterId] : []}
                            onChange={(ids) => setInsertAfterId(ids.length > 0 ? ids[0] : null)}
                            placeholder="Insérer à la fin..."
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-white/5 p-8 rounded-[2rem] border border-white/5 shadow-xl">
                        ...
                        <div className="space-y-4">
                            <div className="bg-slate-950/60 p-1.5 rounded-2xl flex gap-1.5 border border-white/5 shadow-inner">
                                ...
                                <button type="button" onClick={() => setMode('multi')} className={`flex-1 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${mode === 'multi' ? 'bg-emerald-500 text-white' : 'text-slate-500'}`}>Multi</button>
                            </div>
                            ...
                            {mode === 'single' ? (
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div className="group">
                                        <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 ml-1">Effectif</label>
                                ...
                                <div className="space-y-3 max-h-[150px] overflow-y-auto pr-2 custom-scrollbar">
                                    {multiDisciplineData.map((item, idx) => (
                                        <div key={item.id} className="grid grid-cols-[1fr_70px_auto] gap-2 items-end">
                                            <CreatableSearchableSelect label={`Disc. ${idx + 1}`} name={`disc-${item.id}`} value={item.discipline} onChange={(_, v) => updateDiscipline(item.id, 'discipline', v)} options={disciplines} />
                                            <input type="number" value={item.effectif} onChange={e => updateDiscipline(item.id, 'effectif', parseInt(e.target.value) || 1)} className="bg-slate-900 border border-white/10 rounded-xl px-2 py-2 text-white text-xs font-mono" />
                                            <button type="button" onClick={() => removeDiscipline(item.id)} className="p-2 text-red-400 hover:bg-red-500/10 rounded-lg transition-colors"><XCircle size={14} /></button>
                                        </div>
                                    ))}
                                    <button type="button" onClick={addDiscipline} className="w-full py-2 border border-dashed border-white/10 text-slate-500 hover:text-white text-[10px] uppercase font-black">+ Ajouter</button>
                                </div>
                            )}
                        </div>
                    </div>

                    {/* SECTION: ADMIN & HSE TAGS */}
                        ...
                        <div className="bg-slate-950/40 p-6 rounded-[2rem] border border-white/5 shadow-inner space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="AVIS" value={formData['AVIS']} onChange={handleChange} className="w-full bg-slate-900/50 border border-white/10 rounded-xl px-4 py-2 text-white font-mono placeholder:opacity-20" placeholder="AVIS" />
                                </div>
                                <div>
                                    <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1">Numéro OT</label>
                                    <input type="text" name="OT" value={formData['OT']} onChange={handleChange} className="w-full bg-slate-900/50 border border-white/10 rounded-xl px-4 py-2 text-white font-mono placeholder:opacity-20" placeholder="OT" />
                                </div>
                            </div>
                            <div>
                ...
                <footer className="p-8 pt-4 flex justify-end gap-4 bg-slate-900/80 border-t border-white/5 backdrop-blur-md shrink-0">
                    ...
                    <button form="add-task-form" type="submit" className="px-10 py-3 rounded-2xl bg-gradient-to-r from-emerald-600 to-emerald-400 text-white font-black text-[10px] uppercase tracking-widest shadow-xl transition-all hover:scale-[1.02] flex items-center gap-2">
            ...
            {alertConfig.isOpen && (
                <div className="fixed inset-0 bg-slate-950/90 flex items-center justify-center z-[150] p-4">
                    <div className="bg-slate-900 border border-white/10 rounded-[2rem] w-full max-sm overflow-hidden shadow-2xl">
                        <div className="h-1 w-full bg-amber-500"></div>
                        <div className="p-8 space-y-4 text-center">
                            <Activity className="w-12 h-12 text-amber-500 mx-auto" />
                            <h3 className="text-lg font-black text-white uppercase italic">{alertConfig.title}</h3>
                            <p className="text-sm text-slate-400 font-medium">{alertConfig.message}</p>
                            <button onClick={() => setAlertConfig(prev => ({ ...prev, isOpen: false }))} className="w-full py-3.5 rounded-xl bg-white/5 hover:bg-white/10 text-white text-[10px] font-black uppercase tracking-widest transition-all border border-white/10">Compris</button>
                        </div>
                    </div>
                </div>
            )}
